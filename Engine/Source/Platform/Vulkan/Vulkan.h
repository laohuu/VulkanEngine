...
namespace Engine::Utils
{
    ...
    inline void VulkanCheckResult(VkResult result)
    {
        ...
        {
            //            ENGINE_CORE_ERROR("VkResult is '{0}' in {1}:{2}", ::Engine::Utils::VKResultToString(result),
            //            __FILE__, __LINE__);
            if (result == VK_ERROR_DEVICE_LOST)
            {
                using namespace std::chrono_literals;
                //                                std::this_thread::sleep_for(3s);
                //::Engine::Utils::RetrieveDiagnosticCheckpoints();
                //                ::Engine::Utils::DumpGPUInfo();
            }
...
#define VK_CHECK_RESULT(f) \
    { \
        VkResult res = (f); \
        ::Engine::Utils::VulkanCheckResult(res); \
    }

namespace Engine::VKUtils
{
    inline static void SetDebugUtilsObjectName(const VkDevice     device,
