...
namespace Engine
{
    ...
    VulkanPhysicalDevice::VulkanPhysicalDevice()
    {
        ...
        {
            //            ENGINE_CORE_TRACE_TAG("Renderer", "Could not find discrete GPU.");
            selectedPhysicalDevice = physicalDevices.back();
        }

        //        ENGINE_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
        ...
        //        ENGINE_CORE_ASSERT(queueFamilyCount > 0, "");
        ...
        if (extCount > 0)
        {
            ...
            if (vkEnumerateDeviceExtensionProperties(m_PhysicalDevice, nullptr, &extCount, &extensions.front()) ==
                VK_SUCCESS)
            {
                //                ENGINE_CORE_TRACE_TAG("Renderer", "Selected physical device has {0} extensions",
                //                extensions.size());
                for (const auto& ext : extensions)
                {
                    m_SupportedExtensions.emplace(ext.extensionName);
                    //                    ENGINE_CORE_TRACE_TAG("Renderer", "  {0}", ext.extensionName);
                }
            }
        }
        ...
        int requestedQueueTypes = VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_TRANSFER_BIT | VK_QUEUE_COMPUTE_BIT;
        ...
        //        ENGINE_CORE_ASSERT(m_DepthFormat);
    }
    ...
    uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
    {
        ...
        //        ENGINE_CORE_ASSERT(false, "Could not find a suitable memory type!");
        return UINT32_MAX;
    }
        ...
        m_EnabledFeatures(enabledFeatures)
    {
        ...
        // extension
        //        ENGINE_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
...
#if ENGINE_HAS_AFTERMATH
        VkDeviceDiagnosticsConfigCreateInfoNV aftermathInfo = {};
        bool                                  canEnableAftermath =
            enableAftermath &&
        ...
        //        ENGINE_CORE_ASSERT(result == VK_SUCCESS);
    ...
    VkCommandBuffer VulkanDevice::CreateSecondaryCommandBuffer(const char* debugName)
    {
        ...
        //        VKUtils::SetDebugUtilsObjectName(m_LogicalDevice, VK_OBJECT_TYPE_COMMAND_BUFFER, debugName,
        //        cmdBuffer);
        return cmdBuffer;
    }
    ...
    Ref<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
    {
        ...
        //        ENGINE_CORE_VERIFY(m_CommandPools.find(threadID) != m_CommandPools.end());
    ...
    void VulkanCommandPool::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
    {
        ...
        //        ENGINE_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
